...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Listen for changes (SIGNED_IN, TOKEN_REFRESHED, SIGNED_OUT, etc.)
    const { data: sub } = supabase.auth.onAuthStateChange((_event, s) => {
      setSession(s ?? null)
    })

    // On mount & on focus, proactively refresh if near expiry
    ;(async () => {
      const fresh = await getFreshSession(60)
      if (mounted && fresh !== null) setSession(fresh)
    })()
